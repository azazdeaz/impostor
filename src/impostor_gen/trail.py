...
def quadratic_bezier(
    t: float, p0: np.ndarray, p1: np.ndarray, p2: np.ndarray
) -> np.ndarray:
...
class Trail(BaseModel):
    start_point: npt.NDArray[np.float64] = Field(
        default_factory=lambda: np.array([0.0, 0.0, 0.0])
    )
    control_point: npt.NDArray[np.float64] = Field(
        default_factory=lambda: np.array([0.5, 0.0, 1.0])
    )
    end_point: npt.NDArray[np.float64] = Field(
    ...
    def transforms(self) -> list[Transform3D]:
        ...
        if self.steps <= 1:
            ...
            position = quadratic_bezier(
                t, self.start_point, self.control_point, self.end_point
            )
            tangent = quadratic_bezier_derivative(
                t, self.start_point, self.control_point, self.end_point
            )
            rotation = Rotation.align_vectors(
                np.array([[1, 0, 0]]), np.array([tangent])
            )[0]
            transforms.append(
                Transform3D(position=position, rotation=rotation, scale=np.ones(3))
            )
            return transforms
