...
class BezierCurve2D:
    def __init__(
        ...
        self.start_point = np.array(
            [(start_point[0] - x_min) / x_range, start_point[1]]
        )
        self.control_point = np.array(
            [(control_point[0] - x_min) / x_range, control_point[1]]
        )
    ...
    def y_at_x(
        self, x: float, samples: int = 100, tolerance: float = 1e-6
    ) -> Optional[float]:
        """
