...
impl SoftBody {
    pub fn build_stem(
        ...
        //init particles
        for i in 0..n_points {
            let mut point = Vec3::new(0.0, length / (n_points as f32) * (i as f32), 0.0);
        ...
        // edge constraints
        for (p1, p2) in particles.into_iter().tuple_windows() {
            let constraint = EdgeConstraint::from_particles(
                &body.data,
                p1,
                p2,
            );
